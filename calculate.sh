...
psql walmart -c "CREATE OR REPLACE VIEW allblocks_5km AS
...
psql walmart -c "COPY(select * from allblocks_5km) to '`pwd`/build/blocks-to-walmart-5km.csv' WITH CSV HEADER;"

echo "Create view of unique blocks within 5km of a Walmart"
psql walmart -c "CREATE OR REPLACE VIEW unique_blocks_5km AS
    SELECT id, population, min(distance)
    FROM allblocks_5km GROUP BY id, population;"
psql walmart -c "COPY(select * from unique_blocks_5km) to '`pwd`/build/unique-blocks-5km.csv' WITH CSV HEADER;"

echo "Find total number of people within x miles of a Walmart"
psql walmart -c "select (select sum(unique_blocks_5km.population) from unique_blocks_5km) / (select sum(blocks.pop10) from blocks);"
